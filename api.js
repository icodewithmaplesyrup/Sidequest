...
exports.handler = async (event) => {
  ...
  const completionMatch = path.match(/^\/quests\/([^/]+)\/complete$/);
  if (method === 'POST' && completionMatch) {
    const questId = completionMatch[1];
    const quest = quests.find((item) => item.id === questId);
    if (!quest) return json(404, { error: 'Quest not found' });
    quest.completions = (Number(quest.completions) || 0) + 1;
